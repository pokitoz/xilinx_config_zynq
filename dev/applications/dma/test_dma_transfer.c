...
int main() {
    ...
    pl_dma_reset(virtual_sdma_address);
    pl_dma_halt(virtual_sdma_address);

    pl_dma_set_addresses(virtual_sdma_address, SOURCE_MEM_ADDRESS, DEST_MEM_ADDRESS);

	pl_dma_start_channel(virtual_sdma_address);

	pl_dma_set_length(virtual_sdma_address, DATA_TRANSFER_LENGTH);    
    
    printf("Waiting for synchronization\n");
    pl_dma_mm2s_sync(virtual_sdma_address);
    pl_dma_s2mm_sync(virtual_sdma_address); 

    printf("New Dest\n"); 
	print_buffer(virtual_destination_address, DATA_TRANSFER_LENGTH);


    pl_dma_s2mm_status(virtual_sdma_address);
    pl_dma_mm2s_status(virtual_sdma_address);
