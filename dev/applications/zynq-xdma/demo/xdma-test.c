...
/*
	printf("XDMA_GET_NUM_DEVICES\n");
	if (ioctl(fd, XDMA_GET_NUM_DEVICES, NULL) < 0) {
		perror("Error ioctl test transition");
		exit(EXIT_FAILURE);
	}

	printf("XDMA_GET_DEV_INFO\n");
	if (ioctl(fd, XDMA_GET_DEV_INFO, NULL) < 0) {
		perror("Error ioctl test transition");
		exit(EXIT_FAILURE);
	}
*/
	printf("XDMA_TEST_TRANSFER\n");
