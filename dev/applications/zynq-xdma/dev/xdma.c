...
static void xdma_probe(void)
{
	...
	for (;;) {
		...
		if (!tx_chan && !rx_chan) {
			...
			printk(KERN_ERR "Before xdma_add_dev info\n");
			xdma_add_dev_info(tx_chan, rx_chan);
			printk(KERN_ERR "After dma dev info\n");
		}
	}
}
